...
resultspath = 'E:\OneDrive_NTNU\SWwd\FLUENT\AdWell\Particle trajectories\2017 - AB - 2D\archive\170914_2D_C_expVelProf_slip_Carreau_ChhabraUhlherr\exports';
CFDcase = 'C';
...
if todo == 1
    ...
    %% Plot case setup and velocity profiles
    
    
    fig = CreateFigure( fig_titles{2}, '', '' );
    set(fig,'Position',[1 1 29.7 12]);
    axis off;
    fig_sub = tight_subplot(1,2,[.1 .06],[.12 .08],[.06 .02]);    
    ...
    elseif strcmp(CFDcase, 'B')
        sub_title = 'CFD case B (2D)';
    else
    ...
    % Traps
    if strcmp(CFDcase, 'A') || strcmp(CFDcase, 'B')
    ...
    elseif strcmp(CFDcase, 'B')
        text(xmin-0.2,0.5*(ymin+ymax),{'Parabolic velocity profiles with', 'U_f acc. to Khatibi et al. (2016)'},'Rotation',90,'HorizontalAlignment' ,'center','VerticalAlignment','Bottom');
    else
        text(xmin-0.2,0.5*(ymin+ymax),{'Fittted exp. velocity profiles', 'acc. to Khatibi et al. (2016)'},'Rotation',90,'HorizontalAlignment' ,'center','VerticalAlignment','Bottom');
    end
...
%         text(0.5*(xmin+xmax),ymin,{'\color{black}u_s = 0 m/s'},'HorizontalAlignment' ,'left','VerticalAlignment','Top');
    ...
    % Shear rate model
    % text(-0.9,1,'Shear rate formulation: Drag law dependent, see Table 1 for different results presented hereafter.','FontSize',9);
    text(-0.9,1,'Shear rate formulation: ','FontSize',9);
    ...
    text(-0.9,0,'Rheology: Cross (1965), for coefficients see Table 2. ','FontSize',9);
    
    % Drag model
    text(-0.9,-1,'Drag: Schiller & Naumann (1933)','FontSize',9');
    % text(0,0,'Drag: Chhabra & Uhlherr (1980)','FontSize',9');
    % text(0,0,'Drag: Acharya (1976)','FontSize',9');
    % text(-0.9,-1,'Drag: As indicated, see Table 1 for different results presented hereafter.','FontSize',9');
    
    
    % Save figure
    savefig(fig, [resultspath(1:length(resultspath)-7) 'CaseSetup.fig']);
    set(gcf, 'color', 'none');
    set(findall(gcf,'type','axes'), 'color', 'none');
    export_fig CaseSetup.png;
    movefile('CaseSetup.png', resultspath(1:length(resultspath)-7));
	set(gcf, 'color', 'white');
	set(gca, 'color', 'white');
